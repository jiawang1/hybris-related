...
@Controller
@Scope("tenant")
@RequestMapping("/anguo-sellercenter")
public class AnguoSellerCenterController extends AbstractSearchPageController

{

	// CMS Pages

	private static final String SELLERCENTER_CMS_PAGE = "sellercenter";
	private static final String STORECREATION_CMS_PAGE = "sellercenterstorecreation";

	@Resource(name = "sellerCenterBreadcrumbBuilder")
	private ResourceBreadcrumbBuilder sellerCenterBreadcrumbBuilder;

	private static final Logger LOG = Logger
			.getLogger(AnguoSellerCenterController.class);
	...
	//private static final String FORWARD_SHOW_ANGUOSTORE_MANAGE = FORWARD_PREFIX
	//		+ "/anguo-storemanagerment/show-anguostore-manage";
	...
	@RequireHardLogIn
	public String buyerCenter(final Model model)
			throws CMSItemNotFoundException {

		storeCmsPageInModel(model,
				getContentPageForLabelOrId(SELLERCENTER_CMS_PAGE));

		setUpMetaDataForContentPage(model,
				getContentPageForLabelOrId(SELLERCENTER_CMS_PAGE));

		model.addAttribute("breadcrumbs",
				sellerCenterBreadcrumbBuilder.getBreadcrumbs(null));
...
//	public String addStore(final AnguoStoreApplyForm anguoStoreApplyForm,
//			final HttpServletRequest request,
//			final BindingResult bindingResult, final Model model)
//			throws Exception {
		
		public String addStore(final AnguoStoreApplyForm anguoStoreApplyForm,final BindingResult bindingResult,
				final HttpServletRequest request,
				 final Model model)
				throws Exception {
		
		storeCmsPageInModel(model,
				getContentPageForLabelOrId(STORECREATION_CMS_PAGE));
		setUpMetaDataForContentPage(model,
				getContentPageForLabelOrId(STORECREATION_CMS_PAGE));
		model.addAttribute("breadcrumbs", sellerCenterBreadcrumbBuilder
				.getBreadcrumbs("text.sellercenter.storecreation"));
		...
		final CustomerModel customer = (CustomerModel) userService
				.getCurrentUser();
		...
		getAnguoStoreApplyValidator().validate(anguoStoreApplyForm,
				bindingResult);
		...
		if (anguoTempStore != null) {
			...
			// if (StoreApproveStatus.CREATE_APPROVE.equals(approveStatus) ||
			// StoreApproveStatus.MODIFY_APPROVE.equals(approveStatus)) {
	...
	private void buildAnguoStoreApplyForm(final AnguoStoreApplyForm newStore,
			final FileItem item, final String encoding)
	...
	public String anguoStoreapplypage(final Model model) throws CMSItemNotFoundException {
		...
		// getI18NFacade().getRegionsForCountryIso("CN"));
	...
	public RegionResultData getAutocompleteSuggestions(
			final AnguoStoreApplyForm incomingAddressForm, final Model model,
			@RequestParam(value = "cmd", required = false) final String cmd,
			@RequestParam(value = "url", required = false) final String url)
			throws CMSItemNotFoundException {
		final RegionResultData data = new RegionResultData();

		if (incomingAddressForm.getCountryIso() != null
				&& !incomingAddressForm.getCountryIso().isEmpty()) {
			final List<RegionData> regionDTOs = getI18NFacade()
					.getRegionsForCountryIso(
							incomingAddressForm.getCountryIso());
			data.setRegions(regionDTOs);
		}

		if (incomingAddressForm.getRegionIso() != null
				&& !incomingAddressForm.getRegionIso().isEmpty()) {
		...
		if (incomingAddressForm.getCityCode() != null
				&& !incomingAddressForm.getCityCode().isEmpty()
				&& cmd.equals("cityselected")) {
			final List<AbstractLocationItemData> cityDistricts = this
					.getDistrictFacade().getDistrictsByCityCode(
	...
	@RequireHardLogIn
	public String showStoreInfoManage(final Model model)
			throws IllegalAccessException, InvocationTargetException, CMSItemNotFoundException {
		...
		if (anguoStoreTemp != null) {
			...
			if (!StoreApproveStatus.CREATE_REJECT.equals(approveStatus)) {
				...
				storeApplyForm
						.setContactName1(anguoStoreTemp.getContactName1());
				storeApplyForm
						.setContactName2(anguoStoreTemp.getContactName2());
				storeApplyForm
						.setContactName3(anguoStoreTemp.getContactName3());
				storeApplyForm.setContactPhone1(anguoStoreTemp
						.getContactPhone1());
				storeApplyForm.setContactPhone2(anguoStoreTemp
						.getContactPhone2());
				storeApplyForm.setContactPhone3(anguoStoreTemp
						.getContactPhone3());
				...
				storeApplyForm.setRegisterTime(anguoStoreTemp.getRegisterDate()
						.toString());
				// storeApplyForm.setStoreLevel(anguoStoreTemp.getStoreLevel());
				// storeApplyForm.setStoreTemplate(anguoStoreTemp.getStoreTemplate());

				for (final AnguoPlatformServiceModel anguoPlatformServiceModel : anguoStoreTemp
						.getAnguoPlatformService()) {
					platformServiceStr.append(anguoPlatformServiceModel
							.getDescription() + " ");
				}
				storeApplyForm.setAnguoPlatformService(platformServiceStr
						.toString());

			}

			storeApplyForm.setApproveStatus(anguoStoreTemp.getStatus()
					.getCode());
		} else {
		...
		final String fieldsNeedApprove = Config
				.getParameter("anguostore.fields.need.approve");
		...
		if (storeApplyForm.getRegionIso() != null
				&& !(storeApplyForm.getRegionIso().isEmpty())) {
			model.addAttribute(
					"cities",
					getCityFacade().getCitiesByRegionCode(
							storeApplyForm.getRegionIso()));
		}

		if (storeApplyForm.getCityCode() != null
				&& !(storeApplyForm.getCityCode().isEmpty())) {
			model.addAttribute("cityDistricts", getDistrictFacade()
					.getDistrictsByCityCode(storeApplyForm.getCityCode()));
		}
