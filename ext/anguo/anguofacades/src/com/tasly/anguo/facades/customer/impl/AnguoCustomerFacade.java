...
public class AnguoCustomerFacade implements CustomerFacade

{
...

	{
		...
		return getCustomerConverter()
				.convert(getUserService().getCurrentUser());

	}

	@Override
	public void changePassword(final String oldPassword,
			final String newPassword) throws PasswordMismatchException

	{
...

		{
			...
			getCustomerAccountService().changePassword(currentUser,
...

	{
		...
		// Assert.hasText(registerData.getFirstName(),
		// "The field [FirstName] cannot be empty");

		// Assert.hasText(registerData.getLastName(),
		// "The field [LastName] cannot be empty");
		...
		// newCustomer.setName(getCustomerNameStrategy().getName(registerData.getFirstName(),
	...
	@Override
	public void createGuestUserForAnonymousCheckout(final String email,
			final String name) throws DuplicateUidException

	{

		validateParameterNotNullStandardMessage("email", email);

		final CustomerModel guestCustomer = getModelService().create(
				CustomerModel.class);
		...
		guestCustomer
				.setType(CustomerType.valueOf(CustomerType.GUEST.getCode()));

		guestCustomer.setSessionLanguage(getCommonI18NService()
				.getCurrentLanguage());

		guestCustomer.setSessionCurrency(getCommonI18NService()
				.getCurrentCurrency());

		getCustomerAccountService().registerGuestForAnonymousCheckout(
				guestCustomer, guid);

		updateCartWithGuestForAnonymousCheckout(getCustomerConverter().convert(
				guestCustomer));
	...
	protected void setUidForRegister(final RegisterData registerData,
	...
	@Override
	public void updateFullProfile(final CustomerData customerData)
			throws DuplicateUidException
...

	{
		...
		final CustomerModel customerModel = getUserService().getUserForUID(
				uid.toLowerCase(), CustomerModel.class);
...

	{
...

		{
			...
			getCartService().changeCurrentCartUser(
		...
		if (!updateSessionCurrency(userData.getCurrency(),
				getStoreSessionFacade().getDefaultCurrency()))
...

		{
			...
			// Clean the existing info on the cart if it does not beling to the
			// current user
...

			{
				...
				LOG.error(
						"Failed to recalculate order [" + sessionCart.getCode()
								+ "]", ex);
	...
	@Override
	public void updateCartWithGuestForAnonymousCheckout(
			final CustomerData guestCustomerData)

	{
...

		{
			...
			getCartService().changeCurrentCartUser(
					getUserService().getUserForUID(guestCustomerData.getUid()));

		}

		// Update the session currency (which might change the cart currency)

		if (!updateSessionCurrency(guestCustomerData.getCurrency(),
				getStoreSessionFacade().getDefaultCurrency()))
		...
		if (!updateSessionLanguage(guestCustomerData.getLanguage(),
				getStoreSessionFacade().getDefaultLanguage()))
...

		{
			...
			// Clear the delivery address, delivery mode, payment info before
			// starting the guest checkout.
	...
	@Override
	public void rememberMeLoginSuccessWithUrlEncoding(
			final boolean languageEncoding, final boolean currencyEncoding)

	{
		...
		if (!currencyEncoding
				&& !updateSessionCurrency(userData.getCurrency(),
						getStoreSessionFacade().getDefaultCurrency()))
		...
		if (!languageEncoding
				&& !updateSessionLanguage(userData.getLanguage(),
						getStoreSessionFacade().getDefaultLanguage()))
	...
	protected boolean updateSessionCurrency(
			final CurrencyData preferredCurrency,
...

	{
...

		{
			...
			// Get the available currencies and check if the currency iso code
			// is supported

			final Collection<CurrencyData> currencies = getStoreSessionFacade()
					.getAllCurrencies();
	...
	protected boolean updateSessionLanguage(
			final LanguageData preferredLanguage,
			final LanguageData defaultLanguage)

	{
...

		{
			...
			// Get the available languages and check if the language iso code is
			// supported

			final Collection<LanguageData> languages = getStoreSessionFacade()
					.getAllLanguages();
	...
	@Required
	public void setTitleConverter(
			final Converter<TitleModel, TitleData> titleConverter)
	...
	@Required
	public void setCustomerNameStrategy(
	...
	@Required
	public void setCustomerReversePopulator(
			final Populator<CustomerData, UserModel> customerReversePopulator)
